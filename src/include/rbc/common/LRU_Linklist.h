...
private:
    ...
    Mempool *mempool;
public:
    LRU_LIST( Mempool *mempool = NULL ):mempool(mempool){
        length = 0;
        head = NULL;
        tail = NULL;
        _key_to_point_map_size = _key_to_point_map.size();
        mempool->add( "LRU_Linklist", _key_to_point_map_size );
    }
    ~LRU_LIST(){
        ...
        mempool->rm( "LRU_Linklist", _key_to_point_map_size );
        mempool->rm( "LRU_Linklist", sizeof(LRU_LIST_NODE)*length );
    }
    ...
    void remove( LRU_LIST_NODE* key ){
        ...
        mempool->update("LRU_Linklist",  _key_to_point_map_size, _key_to_point_map.size() );
        _key_to_point_map_size = _key_to_point_map.size();
        delete key;
        mempool->rm( "LRU_Linklist", sizeof(LRU_LIST_NODE) );
    }

    LRU_LIST_NODE* insert( const key_type& k ) {
        LRU_LIST_NODE* new_node = new LRU_LIST_NODE();
        mempool->add( "LRU_Linklist", sizeof(LRU_LIST_NODE) );
