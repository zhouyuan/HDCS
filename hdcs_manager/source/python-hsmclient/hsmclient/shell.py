...
from hsmclient import exceptions as exc
import hsmclient.extension
from hsmclient.openstack.common import strutils
from hsmclient import utils
from hsmclient.v1 import shell as shell_v1

DEFAULT_OS_VOLUME_API_VERSION = "1"
DEFAULT_HSM_ENDPOINT_TYPE = 'publicURL'
DEFAULT_HSM_SERVICE_TYPE = 'compute'

logger = logging.getLogger(__name__)


class HsmClientArgumentParser(argparse.ArgumentParser):

    def __init__(self, *args, **kwargs):
        super(HsmClientArgumentParser, self).__init__(*args, **kwargs)
...
class OpenStackHsmShell(object):

    def get_base_parser(self):
        parser = HsmClientArgumentParser(
            prog='hsm',
            description=__doc__.strip(),
            epilog='See "hsm help COMMAND" '
        ...
        parser.add_argument('--version',
                            action='version',
                            version=hsmclient.__version__,
                            help="show program's version number and exit.")

        parser.add_argument('--debug',
                            action='store_true',
                            default=utils.env('HSMCLIENT_DEBUG',
                                              default=False),
        ...
        parser.add_argument('--service-name',
                            ...
                            default=utils.env('HSM_SERVICE_NAME'),
                            help=argparse.SUPPRESS)
        parser.add_argument('--service_name',
                            help=argparse.SUPPRESS)

        parser.add_argument('--hsm-service-name',
                            metavar='<hsm-service-name>',
                            default=utils.env('HSM_VOLUME_SERVICE_NAME'),
                            help=argparse.SUPPRESS)
        parser.add_argument('--hsm_service_name',
                            help=argparse.SUPPRESS)
        ...
        parser.add_argument('--endpoint-type',
                            ...
                            default=utils.env(
                                'HSM_ENDPOINT_TYPE',
                                default=DEFAULT_HSM_ENDPOINT_TYPE),
                            help=argparse.SUPPRESS)
        parser.add_argument('--endpoint_type',
                            help=argparse.SUPPRESS)

        parser.add_argument('--os-hsm-api-version',
                            metavar='<compute-api-ver>',
                            default=utils.env(
                                'OS_VOLUME_API_VERSION',
                                default=DEFAULT_OS_VOLUME_API_VERSION),
                            help=argparse.SUPPRESS)
        parser.add_argument('--os_hsm_api_version',
                            help=argparse.SUPPRESS)
        ...
        parser.add_argument('--insecure',
                            default=utils.env('HSMCLIENT_INSECURE',
                                              default=False),
                            action='store_true',
                            help=argparse.SUPPRESS)
    ...
    def _discover_extensions(self, version):
        ...
        for name, module in itertools.chain(
            ...
            extension = hsmclient.extension.Extension(name, module)
    ...
    def _discover_via_python_path(self, version):
        for (module_loader, name, ispkg) in pkgutil.iter_modules():
            if name.endswith('python_hsmclient_ext'):
    ...
    def main(self, argv):
        ...
        (os_username, os_password, os_tenant_name, os_auth_url,
         ...
         service_type, service_name, hsm_service_name,
         username, apikey, projectid, url, region_name, cacert) = (
             ...
             args.hsm_service_name, args.username,
             args.apikey, args.projectid,
             args.url, args.region_name, args.os_cacert)

        if not endpoint_type:
            endpoint_type = DEFAULT_HSM_ENDPOINT_TYPE

        if not service_type:
            service_type = DEFAULT_HSM_SERVICE_TYPE
    ...
    def do_bash_completion(self, args):
        ...
        hsm.bash_completion script doesn't have to hard code them.
        """
        commands = set()
        options = set()
...
def main():
    try:
        if sys.version_info >= (3, 0):
            OpenStackHsmShell().main(sys.argv[1:])
        else:
            OpenStackHsmShell().main(map(strutils.safe_decode,
                                         sys.argv[1:]))
    except KeyboardInterrupt:
        print("... terminating hsm client", file=sys.stderr)
        sys.exit(130)
