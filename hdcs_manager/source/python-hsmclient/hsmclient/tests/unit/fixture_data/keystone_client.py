...
HSM_ENDPOINT = 'http://www.hsm.com/v1'
...
def _get_normalized_token_data(**kwargs):
    ...
    ref['hsm_url'] = ref.get('hsm_url', HSM_ENDPOINT)
...
def generate_v2_project_scoped_token(**kwargs):
    ...
    o = {'access': {'token': {'id': token,
                              ...
                              'issued_at': '2017-02-08T00:00:00.000000Z',
                              'tenant': {'enabled': True,
    ...
    hsm_endpoint = {
        'endpoints': [
            {
                'publicURL': 'public_' + ref.get('hsm_url'),
                'internalURL': 'internal_' + ref.get('hsm_url'),
    ...
    # Add multiple Hsm endpoints
    for count in range(1, 4):
        # Copy the endpoint and create a service name
        endpoint_copy = copy.deepcopy(hsm_endpoint)
        name = "hsm%i" % count
        ...
        {'endpoints': [
            {
                ...
                'interface': 'public',
                'region': 'RegionOne',
                'url': 'public_' + ref.get('hsm_url')
            },
            {
                'id': uuid.uuid4().hex,
                'interface': 'internal',
                'region': 'RegionOne',
                'url': 'internal_' + ref.get('hsm_url')
            },
            {
                'id': uuid.uuid4().hex,
                'interface': 'admin',
                'region': 'RegionOne',
                'url': 'admin_' + ref.get('hsm_url')
            }],
         'id': uuid.uuid4().hex,
         'type': 'network'},
        {'endpoints': [
