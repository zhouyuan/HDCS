...
function usage {
  echo "Usage: $0 [OPTION]..."
  echo "Run hsmclient's test suite(s)"
  echo ""
...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testropts" = "" ] && [ "$testrargs" = "" ]; then
      ...
      # provided.
      testrargs="discover ./hsmclient/tests"
    fi
  ...
  if [ setup.cfg -nt hsmclient.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='hsmclient/*' --omit='hsmclient/openstack/common/*' -d covhtml -i
  fi
