  ...
  update: function () {
    ...
    if ($rows_to_update.length) {
      ...
      if ($rows_to_update.find('.actions_column .btn-group.open').length) {
        ...
        setTimeout(hsm.datatables.update, interval);
      ...
      $rows_to_update.each(function(index, row) {
        ...
        horizon.ajax.queue({
          ...
          error: function (jqXHR, textStatus, errorThrown) {
            switch (jqXHR.status) {
              ...
              case 404:
                ...
                row_count = hsm.datatables.update_footer_count($table, -1);
          ...
          complete: function (jqXHR, textStatus) {
            ...
            hsm.datatables.validate_button();
            ...
            //setTimeout(hsm.datatables.update, 5000);
          }
        });
      });
      setTimeout(hsm.datatables.update, 4500);
    }
  },
...
hsm.datatables.confirm = function (action) {
  var $action = $(action),
...
hsm.datatables.disable_buttons = function() {
...
hsm.datatables.update_footer_count = function (el, modifier) {
  var $el = $(el),
...
hsm.datatables.add_no_results_row = function (table) {
...
hsm.datatables.remove_no_results_row = function (table) {
 ...
 **/
hsm.datatables.fix_row_striping = function (table) {
  table.trigger('applyWidgetId', ['zebra']);
};

hsm.datatables.set_table_sorting = function (parent) {
...
hsm.datatables.add_table_checkboxes = function(parent) {
...
hsm.datatables.set_table_query_filter = function (parent) {
    ...
    if (input.length > 0) {
      ...
      input.quicksearch(table_selector + ' tbody tr', {
        ...
        onBefore: function () {
          ...
          hsm.datatables.remove_no_results_row(table);
        },
        onAfter: function () {
          var template, table, colspan, params;
          table = $(table_selector);
          hsm.datatables.update_footer_count(table);
          hsm.datatables.add_no_results_row(table);
          hsm.datatables.fix_row_striping(table);
        },
...
hsm.datatables.set_table_fixed_filter = function (parent) {
...
horizon.addInitFunction(function() {
  hsm.datatables.validate_button();
  hsm.datatables.disable_buttons();
  $('table.datatable').each(function (idx, el) {
    hsm.datatables.update_footer_count($(el), 0);
  });
...
//  hsm.datatables.add_table_checkboxes($('body'));
  hsm.datatables.set_table_sorting($('body'));
//  hsm.datatables.set_table_query_filter($('body'));
//  hsm.datatables.set_table_fixed_filter($('body'));
//
//  // Also apply on tables in modal views.
//  horizon.modals.addModalInitFunction(hsm.datatables.add_table_checkboxes);
//  horizon.modals.addModalInitFunction(hsm.datatables.set_table_sorting);
//  horizon.modals.addModalInitFunction(hsm.datatables.set_table_query_filter);
//  horizon.modals.addModalInitFunction(hsm.datatables.set_table_fixed_filter);
//
//  // Also apply on tables in tabs views for lazy-loaded data.
//  //horizon.tabs.addTabLoadFunction(hsm.datatables.add_table_checkboxes);
//  //horizon.tabs.addTabLoadFunction(hsm.datatables.set_table_sorting);
//  //horizon.tabs.addTabLoadFunction(hsm.datatables.set_table_query_filter);
//  //horizon.tabs.addTabLoadFunction(hsm.datatables.set_table_fixed_filter);

  hsm.datatables.update();
});
