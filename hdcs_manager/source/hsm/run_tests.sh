...
function run_tests {
  ...
  if [ setup.cfg -nt hsm.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='hsm/*' --omit='hsm/openstack/common/*' -d covhtml -i
  fi
