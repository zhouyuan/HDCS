...
service_opts = [
    ...
    cfg.StrOpt('hsmapi_storage_listen',
               default="0.0.0.0",
               help='IP address for OpenStack Energy API to listen'),
    cfg.IntOpt('hsmapi_storage_listen_port',
               default=8787,
...
class ProcessLauncher(object):
    ...
    def _wait_child(self):
        ...
        if os.WIFSIGNALED(status):
            ...
            LOG.info('Child %(pid)d killed by signal %(sig)d', locals())
        else:
            code = os.WEXITSTATUS(status)
            LOG.info('Child %(pid)d exited with status %(code)d', locals())
...
class Service(object):
    ...
    def start(self):
        ...
        LOG.audit('Starting %(topic)s node (version %(version_string)s)',
        ...
        # initialize the node into db.
        self.manager.init_node_into_db()
    ...
    def create(cls, host=None, binary=None, topic=None, manager=None,
        ...
        :param topic: defaults to bin_name - 'hsm-' part
        ...
        if not topic:
            topic = binary
        if not manager:
            subtopic = topic.rpartition('hsm-')[2]
