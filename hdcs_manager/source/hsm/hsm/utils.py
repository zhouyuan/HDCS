...
from hsm.openstack.common import excutils
...
from hsm.openstack.common.gettextutils import _
...
def find_config(config_path):
    ...
    :raises: `hsm.exception.ConfigNotFound`

    """
    possible_locations = [
        config_path,
        os.path.join(FLAGS.state_path, "etc", "hsm", config_path),
        os.path.join(FLAGS.state_path, "etc", config_path),
        os.path.join(FLAGS.state_path, config_path),
        "/etc/hsm/%s" % config_path,
    ]
    ...
    def create(self):
        try:
            ...
            if self.password:
                ...
                raise exception.HsmException(msg)
...
def hsmdir():
    import hsm
    return os.path.abspath(hsm.__file__).split('hsm/__init__.py')[0]
...
def monkey_patch():
    ...
    Example: 'hsm.api.ec2.cloud:' \
     hsm.openstack.common.notifier.api.notify_decorator'

    Parameters of the decorator is as follows.
    (See hsm.openstack.common.notifier.api.notify_decorator)
...
def generate_mac_address():
    ...
    #             properly: 0xfa.
    #             Discussion: https://bugs.launchpad.net/hsm/+bug/921838
    mac = [0xfa, 0x16, 0x3e,
...
def gen_ssh_key(username="hsm", hostname=None):
    """ Generate ssh key."""
    try:
        out, _err = execute('hsm-assist', 'checkfile', FLAGS.id_rsa_pub,
                                    run_as_root=True)
