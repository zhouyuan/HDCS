...
from hsm.api import xmlutil
from hsm import exception
from hsm import flags
from hsm.openstack.common import exception as common_exception
...
class ExtensionManager(object):
    ...
    See hsm/tests/api/extensions/foxinsocks/extension.py for an
    example extension implementation.

    """

    def __init__(self):
        LOG.audit('Initializing extension manager.')

        self.cls_list = FLAGS.hsmapi_storage_extension
        self.extensions = {}
    ...
    def register(self, ext):
        ...
        alias = ext.alias
        LOG.audit('Loaded extension: %s', alias)
    ...
    def _check_extension(self, extension):
        ...
        try:
            ...
            LOG.exception("Exception loading extension: %s", unicode(ex))
            return False
    ...
    def _load_extensions(self):
        ...
        old_contrib_path = ('hsm.api.openstack.storage.contrib.'
                            'standard_extensions')
        new_contrib_path = 'hsm.api.contrib.standard_extensions'
        if old_contrib_path in extensions:
            LOG.warn('hsmapi_storage_extension is set to deprecated path: %s',
                     old_contrib_path)
            LOG.warn('Please set your flag or hsm.conf settings for '
                     'hsmapi_storage_extension to: %s', new_contrib_path)
        ...
        for ext_factory in extensions:
            try:
                ...
                LOG.warn('Failed to load extension %(ext_factory)s: '
                         '%(exc)s' % locals())

class ControllerExtension(object):
    """Extend core controllers of hsm OpenStack API.

    Provide a way to extend existing hsm OpenStack API core
    controllers.
    """
...
class ResourceExtension(object):
    """Add top level resources to the OpenStack API in hsm."""
    ...
    for dirpath, dirnames, filenames in os.walk(our_dir):
        ...
        for fname in filenames:
            ...
            try:
                ...
                logger.warn('Failed to load extension %(classpath)s: '
                            '%(exc)s' % locals())
...
def extension_authorizer(api_name, extension_name):
    def authorize(context, target=None):
        ...
        hsm.policy.enforce(context, action, target)
    return authorize
