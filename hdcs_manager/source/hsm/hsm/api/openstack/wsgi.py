...
from hsm.openstack.common import jsonutils
...
class Request(webob.Request):
    ...
    def best_match_content_type(self):
        ...
        if 'hsm.best_content_type' not in self.environ:
            ...
            self.environ['hsm.best_content_type'] = (content_type or
                                                        'application/json')

        return self.environ['hsm.best_content_type']
...
class Resource(wsgi.Application):
    ...
    def _process_stack(self, request, action, action_args,
        ...
        context = request.environ.get('hsm.context')
        ...
        try:
            ...
            msg = ("%(url)s returned with HTTP %(status)d") % msg_dict
        except AttributeError, e:
            msg_dict = dict(url=request.url, e=e)
            msg = ("%(url)s returned a fault: %(e)s") % msg_dict
...
def _set_request_id_header(req, headers):
    context = req.environ.get('hsm.context')
    if context:
